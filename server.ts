...
app.post("/api/chat", async (req: Request<{}, {}, ChatRequestBody>, res: Response) => {
  ...
  try {
    ...
    switch (intent) {
      ...
      case "pricing":
      case "function":
        const { answer: relevantAnswer, relatedQuestions: relevant = [] } = await getRelevantAnswer(
          message,
          updatedHistory,
          intent
        );
      ...
      case "login": // 🆕 追加
        ...
        relatedQuestions = getSuggestedQuestions("login");
        updatedHistory.push({ role: "system", content: "intent:login" });
        break;

      case "difference":
        reply = getDifferenceTemplate().answer;
        relatedQuestions = getSuggestedQuestions("difference");
      ...
      case "recommendation":
        if (!introDone) {
          ...
          if (!team && !purpose) {
            ...
            reply = "ありがとうございます。あわせて主なご利用目的も教えていただけますか？（例：FAQ対応、社内ナレッジ、顧客サポートなど）";
      ...
      case "contract":
        ...
        relatedQuestions = getSuggestedQuestions("contract");
        break;
